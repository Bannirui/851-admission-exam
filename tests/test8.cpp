...
MatrixUDG::MatrixUDG(char* vexs, int vlen, int (*matrix)[9])
{
    ...
    mVexNum = vlen;
    // 初始化顶点
    for (int i = 0; i < mVexNum; i++) mVexs[i] = vexs[i];
    // 初始化边
    for (i = 0; i < mVexNum; i++)
        for (j = 0; j < mVexNum; j++) mMatrix[i][j] = matrix[i][j];
    // 统计边的数量
    for (i = 0; i < mVexNum; i++)
        for (j = 0; j < mVexNum; j++)
            if (i != j && mMatrix[i][j] != INF) mEdgNum++;
    mEdgNum /= 2;
}
MatrixUDG::~MatrixUDG() {}
void MatrixUDG::DFS()
...
void MatrixUDG::prim(int start)
{
    ...
    for (int i = 0; i < mVexNum; i++)
    {
        ...
        for (j = 0; j < mVexNum; j++)
        {
            ...
            if (weights[j] != 0 && mMatrix[k][j] < weights[j]) weights[j] = mMatrix[k][j];
        }
    }
    // 计算最小生成树的权重值
    sum = 0;
    for (int i = 1; i < index; i++)
    {
        ...
        for (j = 0; j < i; j++)
        {
            ...
            if (mMatrix[m][n] < min) min = mMatrix[m][n];
        }
        sum += min;
    }
    // 打印
    std::cout << "PRIM(" << mVexs[start] << ")=" << sum << ": ";
    for (i = 0; i < index; i++) std::cout << prims[i] << " ";
    std::cout << "\n";
}
void MatrixUDG::print()
{
    ...
    for (i = 0; i < mVexNum; i++)
    {
        for (j = 0; j < mVexNum; j++) std::cout << mMatrix[i][j] << " ";
        std::cout << "\n";
    }
}
...
{
    int i;
    if (v <= 0 || v > (mVexNum - 1) || w < 0 || w > (mVexNum - 1)) return -1;
